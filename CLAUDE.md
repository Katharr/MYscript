...
6. 用户要求**模块化**、可持续扩展，并要一个**现代、简约、精致、信息密度适中**的 GUI。
   **界面不写说明性文字（用户拍板）**：不挂页面副标题、不给参数/开关写解释句，界面只留控件名 + 状态 + 错误/安全警告；
   原理、用法、背景一律删（关于页除外）。标定项的一句话「框什么」提示可留。**别往回加。**
...
10. **界面上的「号N」一律显示真实角色名「角色名（等级）」（用户拍板）**：`UserDefault.xml` / `last_server_info`
   ...
   会错配。启动页透明时先扫描；之后按 HWND 缓存顶部姓名区指纹，指纹变化稳定一轮才 OCR。OCR 不确定/名字重复就退回
   「号N」，不猜；OCR 只截屏，不读内存、不注入、不抓包，也无需人工标定。
...
mhxy/
  ...
  gui/
    ...
    float_log.py        悬浮运行日志窗（任意模块脚本一开跑就【自动】收起成它；通用页另有手动入口）：
                        ...
                        App.task_state() 变形（开始=把上次那个模块原样重跑）；自动收起时不抢焦点（约束 9）；
...
## git提交
- 用中文
- 提交摘要和正文不得包含任何隐私信息，包括游戏/角色 ID、账号、角色名、真实姓名、本机路径、设备标识、联系方式或可关联身份的截图内容；使用通用描述或已脱敏占位符。
