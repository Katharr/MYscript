...
TEAM_CALIBRATION = {
    "regions": [
        ("team_panel", "队伍面板区", "创建/申请/接受 按钮都在这里找"),
        ("friend_list", "好友列表区", "滚轮在此翻找队长ID"),
    ],
    "templates": [
        ("team_create", "「创建队伍」按钮", "队伍面板里的「创建队伍」"),
        ("team_apply", "「申请」标签页", "点它才看得到入队申请列表"),
        ("team_accept", "「同意」按钮", "申请列表里那行右侧的「同意」，见即点"),
        ("team_apply_join", "「申请入队」按钮", "队员点队长右侧箭头后弹出的按钮"),
        ("team_arrow", "队长ID右侧箭头(可选)", "好友列表里ID右侧的箭头；不标则用固定偏移兜底"),
        ("leader_id", "队长ID", "队员据此定位队长，框队长名字、要独特"),
        ("team_quit", "解散·「退出队伍」按钮", "队伍面板里的「退出队伍」"),
    ],
    "watchlist": False,
}
