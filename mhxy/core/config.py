...
DEFAULT_CONFIG = {
    ...
    "window_process": "MyTabCtrl_x64r.exe,MyGame_x64r.exe",
                                         # 只认这些进程的窗口：标题会和终端/编辑器等撞，进程名才稳
                                         #   （类名是随机串无法白名单）。多个用逗号分隔；空串=退回纯标题匹配。
                                         #   2026-09 更新后可见窗口改由标签外壳 MyTabCtrl_x64r.exe 持有，
    ...
    "tasks": {
        ...
        "launch_login": {
            ...
            "loop": {
                ...
                "recheck_previous_after_sec": 3.0,  # 进入某步后，多久才开始回查上一步
        ...
        "treasure_map": {
            ...
            "loop": {
                ...
                "activity_columns": 2,       # 活动列表每排几张卡片（仅供参考；定位「参加」已改为按条目几何绑定，
                                             #   见 core/list_row.py——不再靠列等分猜卡片右缘）
                "join_min_score": 0.7,       # 「参加」按钮的接受下限。参加是小字按钮，同一颗按钮因所在卡片底色/
                                             #   进度文字不同，分数会在 0.7~1.0 之间飘——故它【不照 match_threshold
                                             #   一刀切】，而是在「条目所在行的右侧」这条几何带里取最高分，
                                             #   只要 ≥ 本项就接受（认不出的那个号看日志里那行实得分再调它）。
                                             #   条目（卡片图标）本身仍照 match_threshold 卡，见 core/list_row.py。
                "join_reach_ratio": 12.0,    # 「参加」横向搜索范围：条目中心往右最多「条目模板宽 × 本值」
                                             #   （默认 12：实测够到本卡按钮，又收在邻卡按钮之前）。换布局后先调它
                "join_band_ratio": 3.2,      # 「参加」纵向搜索带高 = 条目模板高 × 本值（罩住本卡、不探进上下邻卡）
        ...
        "taohaiqu": {
            ...
            "loop": {
                ...
                "activity_columns": 2,       # 活动列表每排几张卡片（仅供参考；定位「参加」已改为按条目几何绑定，
                                             #   见 core/list_row.py——不再靠列等分猜卡片右缘）
                "join_min_score": 0.7,       # 「参加」按钮的接受下限（小字按钮分数会飘，故不照 match_threshold 一刀切）
                "join_reach_ratio": 12.0,    # 「参加」横向搜索范围：条目中心往右最多「条目模板宽 × 本值」
                "join_band_ratio": 3.2       # 「参加」纵向搜索带高 = 条目模板高 × 本值（罩住本卡、不探进上下邻卡）
            },
            "regions": {                 # 相对游戏窗口 [x,y,w,h]，标定向导写入
