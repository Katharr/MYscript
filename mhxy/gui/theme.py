...
# ---- 标定尺寸组（「胶囊组」）配色 ----
# 语义：一个「尺寸组」= 某一套标定图是在哪个游戏窗口尺寸下标定的。全局共用同一套模板图
# （绝不按尺寸复制模板文件），只在 config 里用【指针】记录「这张图属于哪一组」。
# 组是只增不减的（最多 MAX_PROFILES 组），编号从 ① 递增，颜色按 index 取模循环——
# 故【编号才是主键，颜色只是辅助识别】。最多 3 组时颜色天然不重复。
# 每个元素是 (底色, 文字/描边色)，都是 (light, dark) 二元组，与上文令牌同构。
# 加/改配色只动这里即可整体换肤（遵守本文件既有约定）。
PROFILE_COLORS = [
    (("#dbe7ff", "#17305c"), ("#2f6fed", "#6ea4ff")),   # ① 蓝（同 ACCENT 家族）
    (("#dff3e8", "#15301f"), ("#1f9d6b", "#3ecf8e")),   # ② 绿（同 SUCCESS 家族）
    (("#fdf0d8", "#3a2c10"), ("#c98a16", "#f2b34b")),   # ③ 橙（同 WARN 家族）
]
PROFILE_NEUTRAL = (("#e9ebef", "#2a2f3a"), ("#6b7280", "#8a93a3"))  # 未记录组时用（灰）
# 圈码：编号 → 显示字符。编号超过表长则退回阿拉伯数字（不可能，最多 3 组，留作兜底）。
PROFILE_NUMERALS = "①②③④⑤⑥⑦⑧⑨"
