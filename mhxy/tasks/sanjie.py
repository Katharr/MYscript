...
完整流程：
  开活动 → 找三界奇缘条目 → 点参加
  → 点击10次选择题答案（只选第一个）→ 领取奖励 → 关闭

注意：三界奇缘是11:00之后才有的活动。
...
S_FIND_CARD = "FIND_CARD"           # 滚轮找三界奇缘条目 → 点参加
S_ANSWER = "ANSWER"                 # 点击选择题答案
S_CLAIM_REWARD = "CLAIM_REWARD"     # 领取奖励
S_CLOSE_PANEL = "CLOSE_PANEL"       # 关闭面板

_REQUIRED_FLAGS = ["sanjie_entry", "activity_join", "sanjie_answer", "sanjie_close"]
...
class SanjieTask(Task):
    name = "sanjie"
    title = "三界奇缘"
    description = "自动完成三界奇缘：开活动→参加→答题10次→领奖→关闭"
    is_dungeon = False
    ...
    CALIBRATION = {
        ...
        "templates": [
            *Task.BASE_CALIBRATION_TEMPLATES,
            ("sanjie_entry", "三界奇缘入口", "活动列表里「三界奇缘」那一条，框图标+文字（左侧），不要框参加按钮"),
            ("sanjie_answer", "选择题答案(第一个)", "选择题的第一个选项按钮（只选第一个，不管对错）"),
            ("sanjie_claim", "领取奖励按钮", "答题完成后的领取奖励按钮"),
            ("sanjie_close", "关闭按钮", "面板右上角的关闭按钮（X 或 关闭）"),
        ],
        "watchlist": False,
    }
    ...
    def preflight(self, ctx):
        ...
        if not wins:
            problems.append("没找到/没选中目标窗口 —— 请先「选择窗口」选好要跑三界奇缘的号")
    ...
    def run(self, ctx):
        ...
        answer_count = loop.get("answer_count", 10)
        ...
        ctx.log(f"开始三界奇缘任务（目标答题 {answer_count} 次，{len(wctxs)} 个号）…")
        ...
        rotation.run_rotation(self._make_rotation(
            ctx, records,
            lambda rec: self._step_once(rec["ctx"], rec, loop, regions, threshold, answer_count),
    ...
    def make_chain_driver(self, wctx):
        ...
        rec = {"ctx": wctx, "state": S_OPEN_ACTIVITY, "t_state": time.time(),
               "answers_done": 0, "done": False, "dead_logged": False}
        return rec, (lambda: self._step_once(wctx, rec, loop, regions, threshold, answer_count))

    def _step_once(self, ctx, rec, loop, regions, threshold, answer_count):
        st = rec["state"]
        if st == S_OPEN_ACTIVITY:
            ...
            self._do_answer(ctx, rec, loop, threshold, answer_count)
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if list_region:
                ...
                if scene is not None:
                    for flag_key in ["sanjie_entry", "activity_join"]:
                        ...
                        if tpl is not None:
                            ...
                            if hit:
                                ...
                                return
                    ctx.log("活动列表已打开（截图成功，未立即找到三界奇缘卡片）。", level="info")
                    self._goto(rec, S_FIND_CARD)
                    return
            else:
    ...
    def _do_find_card(self, ctx, rec, loop, regions, threshold):
        """滚轮找三界奇缘条目 → 点参加"""
        list_region = regions.get("activity_list")
        entry_tpl = self.flags.get("sanjie_entry")
        join_tpl = self.flags.get("activity_join")

        if entry_tpl is None:
            ctx.log("❌ sanjie_entry 模板未加载。", level="error")
            rec["done"] = True
            return
        if join_tpl is None:
        ...
        # 调试：输出活动列表区域信息
        if list_region:
            ctx.log(f"📍 活动列表区域: {list_region}", level="info")
        else:
            ctx.log("⚠ 活动列表区域未标定，将使用整个窗口", level="warn")
        ...
        def probe(scene, rect):
            ...
            if hit is None:
                ...
                if hit_low:
                    cx, cy, score = hit_low
                    ctx.log(f"⚠ 低阈值(0.70)匹配到疑似「三界奇缘」（{score:.3f}<{threshold}），建议降低阈值。", level="warn")
                else:
                    ctx.log(f"❌ 未找到「三界奇缘」（阈值 {threshold}），继续滚动…", level="info")
                return scan.SCROLL, None

            cx, cy, score = hit
            entry_xy = (rect[0] + cx, rect[1] + cy)
            ctx.log(f"✓ 找到「三界奇缘」图标（{score:.3f}），坐标 {entry_xy}，开始找「参加」按钮…", level="info")

            join = self._find_join_on_row(ctx, list_region, entry_xy, threshold, loop,
                                            "activity_join", "sanjie_entry")
            if join is not None:
                ctx.mouse.click(join[0], join[1])
                ctx.log(f"找到「三界奇缘」→ 点「参加」（{join[2]:.3f}）。", level="hit")
                return scan.ACCEPT, join

            ctx.log("认出「三界奇缘」但没找到「参加」（检查 activity_join 模板/阈值）。", level="warn")
            return scan.STAY, None

        ctx.log(f"🔍 开始在活动列表搜索「三界奇缘」（阈值 {threshold}）…")
        ...
        if res.stopped:
            return
        ctx.log("活动列表里翻找「三界奇缘」多次未果。", level="warn")
        rec["done"] = True

    def _do_answer(self, ctx, rec, loop, threshold, answer_count):
        """点击选择题答案（只选第一个）"""
        # 检查是否有答案按钮
        answer_tpl = self.flags.get("sanjie_answer")
        if answer_tpl is None:
            ctx.log("❌ sanjie_answer 模板未加载。", level="error")
            rec["done"] = True
            return

        scene_rect = self._scene_rect(ctx, ctx.task_cfg(self.name).get("regions", {}))
        cur = win_mod.grab(scene_rect)
        hit = self._match_scene(cur, scene_rect, "sanjie_answer", threshold)
        ...
        if hit is not None:
            ...
            rec["answers_done"] += 1
            ctx.log(f"点击答案（{score:.3f}），第 {rec['answers_done']} 次点击。", level="hit")
            self._interruptible_sleep(ctx, 1.0)  # 等待下一题出现
            # 重置超时计时
            rec["t_state"] = time.time()
        else:
            # 找不到答案按钮，检查是否已答完
            if self._state_elapsed(rec) > 15:
                # 超时没找到答案，可能已答完，尝试领取奖励
                ctx.log("未找到答案按钮，可能已答完，尝试领取奖励。", level="info")
    ...
    def _do_claim_reward(self, ctx, rec, threshold):
        """领取奖励"""
        claim_tpl = self.flags.get("sanjie_claim")
        ...
        scene_rect = self._scene_rect(ctx, ctx.task_cfg(self.name).get("regions", {}))
        cur = win_mod.grab(scene_rect)
        hit = self._match_scene(cur, scene_rect, "sanjie_claim", threshold)
        if hit is not None:
    ...
    def _do_close_panel(self, ctx, rec, threshold):
        ...
        scene_rect = self._scene_rect(ctx, ctx.task_cfg(self.name).get("regions", {}))
        cur = win_mod.grab(scene_rect)
        hit = self._match_scene(cur, scene_rect, "sanjie_close", threshold)
        if hit is not None:
            ctx.mouse.click(hit[0], hit[1])
