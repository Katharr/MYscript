...
class SecretRealmTask(Task):
    ...
    CALIBRATION = {
        "regions": [
            ("scene", "主识别区", "留空=整窗检测(推荐)", True),
            ("activity_list", "活动列表区域", "滚轮在此翻找秘境降妖卡片"),
        ],
        "templates": [
            ("sr_entry", "活动卡片入口", "要点「参加」的那张卡片，框图标+文字"),
            ("sr_join", "参加按钮", "那张卡片右侧的「参加」"),
            ("sr_select", "「秘境降妖」选项", "点参加后弹框里的那个选项"),
            ("sr_dungeon_enter", "「进入」按钮(选副本，可选)", "选副本界面左下角的「进入」，没有可不标"),
            ("sr_confirm", "「确定」按钮(选副本后才有，可选)", "选完副本后的「确定」，没有可不标"),
            ("sr_continue", "「继续挑战」按钮", "确定后出现的「继续挑战」"),
            ("sr_challenge", "「挑战」按钮", "「继续挑战」后的「挑战」，点它开始战斗"),
            ("sr_enter_battle", "「进入战斗」按钮", "难度关卡处手动点的「进入战斗」"),
            ("sr_leave", "「离开」按钮", "失败/超时后点它退出秘境"),
            ("sr_fail", "「失败」标志(可选)", "战斗失败的「失败」字样/弹窗（先点它结算）"),
            ("sr_battle", "战斗界面标志(可选)", "战斗独有元素，仅用于日志诊断"),
        ],
        "watchlist": False,
    }
    ...
    def _do_find_card(self, ctx, rec, loop, regions, threshold):
        ...
        def probe(scene, rect):
            ...
            got = list_row.locate_card(scene, rect, self.flags.get("sr_entry"),
                                       self.flags.get("sr_join"), threshold,
            ...
            if got.join_x is None:
                ctx.log("认出卡片但右侧没找到「参加」——按上面那行『最佳候选 xx < 下限 yy』判断："
                        "差一点点就把 tasks.secret_realm.loop.join_min_score 调低些，差很多就重标 sr_join 模板。"
                        "原地重试、不滚动。", level="warn")
                return scan.STAY, None
            ctx.mouse.click(got.join_x, got.join_y)
            ctx.log(f"找到卡片（{got.anchor_score:.3f}）→ 点「参加」（{got.join_score:.3f}），等对话框。",
                    level="hit")
