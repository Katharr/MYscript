...
class EscortTask(Task):
    ...
    CALIBRATION = {
        "regions": [
            ...
            ("activity_list", "活动列表区域", "滚轮在此翻找「运镖」条目"),
        ],
        "templates": [
            ("escort_entry", "运镖入口", "「运镖」那一条，框图标+文字"),
            ("escort_join", "参加按钮", "「运镖」那行右侧的「参加」"),
            ("escort_silver", "「押送普通镖银」按钮", "弹出对话框里要点的那个按钮"),
            ("escort_confirm", "「确认」按钮", "上一按钮后的确认按钮"),
            ("escort_ongoing", "「运镖中」标志", "运镖途中常驻屏幕的标志，框它独特的部分"),
            ("escort_battle", "战斗界面标志(可选)", "战斗独有元素，防战斗期被误判结束"),
        ],
        "watchlist": False,
    }
